...
import argparse
from model import Swin_Classifier
from dataset import PE_dataset
...
def train(rank, world_size, args, ddp_model_parallel, training_Set, valid_Set, optimizer, loss_function):
    ...
    for epoch in range(args.epochs):  # loop over the dataset multiple times
        ...
        for i_batch, data in enumerate(pbar):
            input, label = data[0].to(rank), data[1].to(rank)            
            label = label.to(torch.float32)
            optimizer.zero_grad(set_to_none=True)
            with torch.amp.autocast(device_type='cuda',  dtype=amp_dtype, enabled=True):
                output = ddp_model_parallel(input) # this is for inhale + exhale
...
def test(args, test_loader, loss_function, device):
    model = Swin_Classifier(in_channels=1, n_class=1)
    ...
    with torch.no_grad():
        ...
        row = [
            "test", "SwinUNETR_Inhale", args.current_fold, test_auc, sensitivity, specificity, ppv, npv, f1,
        ]
...
def main(rank, world_size, args, rskf, dataset, labels):
    ...
    for train_fold_ind, test_fold_ind in rskf.split(np.zeros(len(dataset)), labels):
        ...
        args.save_path = os.path.join(
            './', 
            f'model_SwinUENTR_Inhale_fold{c}.pt'
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--N_rounds", required=True, type=int, help="Current fold for training")
    ...
    dataset = PE_dataset(label_file=label_file, img_dir=img_dir)
