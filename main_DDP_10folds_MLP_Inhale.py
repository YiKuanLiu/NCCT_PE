...
def train(rank, world_size, args, ddp_model_parallel, training_Set, valid_Set, optimizer, loss_function):
    ...
    for epoch in range(args.epochs):  # loop over the dataset multiple times
        ...
        for i_batch, data in enumerate(pbar):
            ...
            output = ddp_model_parallel(input1) # only the inhale
...
def test(args, test_loader, loss_function, device):
    ...
    model = MLPClassifier(input_dim=101, hidden_dims=hidden_dims, num_classes=1, dropout = args.dropout_rate)
    ...
    with torch.no_grad():
        ...
        row = [
            "test", "MLP_inhale", args.current_fold, test_auc, sensitivity, specificity, ppv, npv, f1,
        ]
